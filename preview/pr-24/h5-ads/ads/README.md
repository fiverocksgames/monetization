...
into the adopting game's development-only preview asset output **only for the local-file mode**. For shared static hosting, pass `mockVideoUrl` (see below) and do not copy the MP4. Check the actual URL, MIME type, playback and license/provenance for either mode. The Mock overlay has
...
For the Match Fix developer-facing React/Vite source-porting guide, see
[`docs/guides/MATCH_FIX_H5_MONETIZATION_PORTING.md`](../../../docs/guides/MATCH_FIX_H5_MONETIZATION_PORTING.md).
This is a development-only guide, not an npm SDK release or production ads authorization.

## Shared static MP4 (optional development-only mode)

A game can **stream** the common creative from an approved HTTPS static site
instead of copying `265552.mp4` into its own repository or preview bundle.
The game still source-ports the four JS modules. Use an explicitly configured
`mockVideoUrl` with a verified publicly served URL:

```js
const ads = new AdsService({
  environment: buildAdEnvironment, // Explicit development preview only.
  publisherId: '',
  manifestUrl: './assets/mock/manifest.json',
  fallbackAds: [{
    id: 'ad-001', video: 'assets/mock/265552.mp4',
    clickUrl: 'https://fiverocksgames.github.io/game-hub/',
    formats: ['rewarded', 'interstitial'], weight: 1
  }],
  mockVideoUrl: approvedPublicVideoUrl // Verified https://.../*.mp4 URL
});
```

`mockVideoUrl` overrides the selected mock's **video source only** and is
assigned directly to the HTML video element. There is no MP4 `fetch`, binary
ingress, or game-local copy. It must be HTTPS, have an `.mp4` path and contain
no URL credentials/query/fragment; an invalid nonempty override fails closed.
The manifest and its click-through destination remain independent. For a
zero-copy integration, retain a development-only local manifest or `fallbackAds`
metadata; never depend on fetching cross-origin JSON unless the static server
has been checked for CORS. A missing or unplayable remote video returns no reward.

**Publishing prerequisite:** This SDK option does not itself deploy or
validate the public MP4. The static host URL must be observed to return the
intended ~30s original with correct MIME/range delivery and documented
provenance/rights before changing a game to that URL. Avoid temporary PR-preview
URLs, which can disappear; prefer a stable approved asset URL. Static assets
are public, so do not publish a creative without distribution rights.
The SDK must never enable mock in production or substitute mock for an actual
started Google ad.
